...
const Tam = () => {
    ...
    const fetchData = async (categoryID) => {
        try {
            const response = await fetchPictureArtByGroup(categoryID);
            ...
            <div className="w-full h-[100px] flex justify-between items-center pr-16 pl-16 bg-[#27242E]">
                {categories.map((category) => (
                    <span
                        ...
                        onClick={() => setSelectedCategoryID(category.id)} // Cập nhật ID khi nhấn vào span
                    >
                        {category.name}
                    </span>
                ))}
                    ...
                    data.map((categoryData) => (
                        categoryData.pictureArtList?.map((iten) => (
                            ...
                            transform hover:scale-[1.02] hover:shadow-lg transition-all duration-300' >
                                <img src={API_URL_IMAGE + iten.thumb}
                                    alt={`img-${iten.id}`}
                                    className="object-cover w-full h-auto"></img>
